    ...
    <div class="px-8 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-2 text-sm">
                <a href="{{ route('admin.dashboard') }}" class="text-gray-500 hover:text-[#5B3DF5] font-medium transition-colors">
                    Dashboard
                </a>
                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
                <span class="text-gray-900 font-semibold">{{ $title ?? 'Admin' }}</span>
            </div>
        </div>
        ...
        <div class="flex items-center gap-6">
            {{-- Date Badge --}}
            <div class="hidden lg:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#5B3DF5]/5 to-[#5B3DF5]/10 rounded-full border border-[#5B3DF5]/20">
                <svg class="w-4 h-4 text-[#5B3DF5]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                <span class="text-sm font-semibold text-[#5B3DF5]">{{ now()->format('d M Y') }}</span>
            </div>

            {{-- Notifications --}}
            <button class="relative p-2 text-gray-400 hover:text-[#5B3DF5] transition-colors rounded-lg hover:bg-[#5B3DF5]/5">
            ...
            <div class="flex items-center gap-3 pl-6 border-l border-gray-200">
                ...
                <div class="relative group">
                    <div class="w-11 h-11 rounded-full bg-gradient-to-br from-[#5B3DF5] to-[#7C5CFF] flex items-center justify-center text-white font-bold shadow-lg cursor-pointer transition-transform hover:scale-105">
                    ...
                    <div class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right">
                        <div class="p-3 border-b border-gray-100">
                            ...
                            <p class="text-xs text-gray-500">{{ auth()->user()->email ?? 'admin@didispen.id' }}</p>
                        </div>
                        <div class="p-2">

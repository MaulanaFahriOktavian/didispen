<aside class="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-white border-r border-[#E5E7EB] z-20">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto scrollbar-hide">
        <p class="px-3 text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-2">Menu Satpam</p>
        
        <a href="{{ route('satpam.dashboard') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('satpam.dashboard') ? 'bg-[#5B3DF5]/10 text-[#5B3DF5]' : 'text-[#6B7280] hover:bg-[#F8FAFC] hover:text-[#111827]' }} transition-colors">
    ...
    <div class="border-t border-[#E5E7EB] p-4">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-semibold text-[#111827] truncate">{{ auth()->user()->username ?? 'Satpam' }}</p>
                <p class="text-xs text-[#6B7280] truncate">Security Guard</p>
            </div>

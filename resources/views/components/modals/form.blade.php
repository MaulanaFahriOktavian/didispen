    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div x-show="open" 
                 ...
                 {{ $attributes->merge(['class' => "relative transform overflow-hidden rounded-[20px] bg-white text-left shadow-xl border border-[#E5E7EB] transition-all sm:my-8 sm:w-full {$sizeClasses}"]) }}>
                ...
                <div class="bg-[#F8FAFC] px-6 py-4 border-t border-[#E5E7EB] flex items-center justify-end gap-3">
                    <x-ui.button variant="ghost" @click="open = false">Batal</x-ui.button>
                    <x-ui.button variant="primary" type="submit" form="{{ $name }}-form">{{ $submitText }}</x-ui.button>
                </div>
            </div>
        </div>
    </div>
